---
name: qa-only
description: Use when asked to "just report bugs", "qa report only", or "test but
  don't fix". Report-only QA — tests a web application and produces a structured
  health report with score, screenshots, and repro steps. Never fixes anything.
  For the full test-fix-verify loop, use /qa instead. Proactively suggest when
  the user wants a bug report without code changes. (nstack)
allowed-tools:
  - Bash
  - Read
  - Write
...
```
[nstack] No browser available. qa-only requires either:
...
1. Detect browse binary (see Setup above)
...
12. **Never refuse to use the browser.** When the user invokes /qa-only, they are requesting browser-based testing. Never suggest evals, unit tests, or other alternatives as a substitute. Even if the diff appears to have no UI changes, backend changes affect app behavior — always open the browser and test.
