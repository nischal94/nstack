---
name: canary
description: Use when asked to "monitor deploy", "canary", "post-deploy check",
  "watch production", or "verify deploy". Post-deploy canary monitoring — watches
  the live app for console errors, performance regressions, and page failures.
  Takes periodic screenshots and compares against pre-deploy baselines. (nstack)
allowed-tools:
  - Bash
  - Read
  - Write
...
```bash
mkdir -p .nstack/canary
mkdir -p .nstack/canary/baselines
mkdir -p .nstack/canary/screenshots
```
...
```bash
$B goto <page-url>
$B snapshot -i -a -o ".nstack/canary/baselines/<page-name>.png"
$B console --errors
$B perf
$B text
```
...
Save the baseline manifest to `.nstack/canary/baseline.json`:
...
```bash
$B goto <page-url>
$B snapshot -i -a -o ".nstack/canary/screenshots/<page-name>-<check-number>.png"
$B console --errors
$B perf
```
...
1. **Page load failure** — navigate returns error or timeout → CRITICAL ALERT
...
Screenshots:   .nstack/canary/screenshots/
